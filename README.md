...
- The app is command-line only.
- There is no user authentication.
- There is no database.
- XML files are local files only.
- The CLI is simple and does not support editing existing task text.
- Ids are generated in memory, so after loading an XML file a new session may create ids that overlap with loaded ids.

## Future Work

- Add edit and delete commands.
- Add search by title or priority.
- Add better id recovery after loading XML.
- Add CSV export.
- Add a small GUI.
- Add Mockito tests for service dependencies.

## Interview Notes

Possible questions and short answers:

### Why Composite?

Because a task group and a task must be handled in a similar way. A group can contain tasks or other groups, so a tree structure fits well.

### Why Factory?

Because object creation should not be spread across the CLI and service. The factory creates valid objects with generated ids.

### Why XML?

Because it uses Java standard APIs and supports structured data. It is better than manual string parsing.

### Why Exception Shielding?

Because users should not see stack traces or internal exception details. The app returns safe messages instead.

### Why Strategy?

Because sorting by date, priority, or title can change without changing the service code.

### Why Observer?

Because the service should not know who reacts to events. A listener can be added later without changing the service workflow.

### Why Chain of Responsibility?

Because input validation has several independent rules. Splitting them makes the code easier to extend.

### Why Reflection and Custom Annotations?

Because text validation can be declared on input fields and handled in one reusable validator.

### What would you improve first?

Id handling after loading XML. The current generator starts fresh each run, so a loaded project could overlap with newly generated ids.
