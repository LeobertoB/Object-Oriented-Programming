...
public final class TaskComponentFactory {
    ...
    public Task createTask(String title, String description, Priority priority, LocalDate dueDate) {
        Map<String, String> safeText = AnnotatedInputValidator.sanitize(new TextInput(title, description));
        Priority safePriority = Objects.requireNonNull(priority, "priority cannot be null");
        LocalDate safeDueDate = Objects.requireNonNull(dueDate, "dueDate cannot be null");
